...
#define MAX_FLOAT 3.40282347e+18F 
...
struct vector3
{
	...
	vector3(float Nowx, float Nowy, float Nowz)
	{
		x = Nowx;
		y = Nowy;
		z = Nowz;
...
vector3 vec3_scale(Real s, const vector3* v);
...
GLG_INLINE vector3 vec3_add(const vector3* v0, const vector3* v1)
{
	vector3 dst;
	dst.x = v0->x + v1->x;
	dst.y = v0->y + v1->y;
...
GLG_INLINE vector3 vec3_scale(Real s, const vector3* v)
{
	vector3 r;
	r.x = s*v->x;
	r.y = s*v->y;
...
GLG_INLINE vector3 invert(vector3 origin)
{
	vector3 result;
	result.x = -origin.x;
	result.y = -origin.y;
